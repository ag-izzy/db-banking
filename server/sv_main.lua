if Config.Framework == 'esx' then
    ...
    RegisterNetEvent('db-banking:withdraw', function(amount)
        ...
        if xPlayer.getAccount('bank').money >= amount then
            ...
            InsertInDatabase(xPlayer.getIdentifier(), '- '..amount..'$ Withdrew')
        end
    end)
    ...
    RegisterNetEvent('db-banking:deposit', function(amount)
        ...
        if xPlayer.getAccount('money').money >= amount then
            ...
            InsertInDatabase(xPlayer.getIdentifier(), '- '..amount..'$ Deposited')
        end
    end)
    ...
    RegisterNetEvent('db-banking:transfer', function(data)
        ...
        if xPlayer.getAccount('bank').money >= data.amount then
            ...
            InsertInDatabase(xTarget.getIdentifier(), '+ '..data.amount..'$ from'..xPlayer.firstName..' '..xPlayer.variables.lastName)
            InsertInDatabase(xPlayer.getIdentifier(), '- '..data.amount..'$ to'..xTarget.firstName..' '..xTarget.lastName)
        end
    end)
...
elseif Config.Framework == 'qb' then
    ...
    RegisterNetEvent('db-banking:withdraw', function(amount)
        ...
        if Player.Functions.GetMoney('bank') >= amount then
            ...
            InsertInDatabase(Player.PlayerData.license, '- '..amount..'$ Withdrew')
        end
    end)
    ...
    RegisterNetEvent('db-banking:deposit', function(amount)
        ...
        if Player.Functions.GetMoney('money') >= amount then
            ...
            InsertInDatabase(Player.PlayerData.license, '- '..amount..'$ Deposited')
        end
    end)
    ...
    RegisterNetEvent('db-banking:transfer', function(data) --TOOD Post Transfers Vals And Use The Data For This
        ...
        if Player.Functions.GetMoney('bank') >= data.amount then
            ...
            InsertInDatabase(Target.PlayerData.license, '+ '..data.amount..'$ from'..PlayerData.firstName..' '..PlayerData.lastName)
            InsertInDatabase(Player.PlayerData.license, '- '..data.amount..'$ to'..TargetData.firstName..' '..TargetData.lastName)
        end
    end)
end
